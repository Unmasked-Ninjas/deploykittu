...
export default function PigeonJourneySection() {
  ...
  return (
    <section
      ref={containerRef}
      className="h-screen w-full overflow-hidden bg-[#0A0816] relative"
    >
      <div
        ref={sceneRef}
        className="h-full w-[500vw] flex absolute top-0 left-0"
      >
        {/* Zone 1: Nepal Mountains (Dark Indigo) */}
        <div className="w-[100vw] h-full relative bg-gradient-to-b from-[#0A0816] to-[#1A0A2E]">
          <div
            className="absolute bottom-0 w-full h-[60%] bg-[#0A1E2E]"
            style={{
              clipPath:
                "polygon(0% 100%, 0% 50%, 20% 20%, 40% 60%, 60% 10%, 80% 50%, 100% 30%, 100% 100%)",
            }}
          ></div>
          <div
            className="absolute bottom-0 w-full h-[40%] bg-[#1A0A2E]"
            style={{
              clipPath:
                "polygon(0% 100%, 0% 60%, 30% 30%, 50% 70%, 70% 40%, 100% 50%, 100% 100%)",
            }}
          ></div>
          {/* Snowcaps styled darkly */}
          <div
            className="absolute bottom-[40%] left-[10%] w-[20%] h-[20%] bg-[#7A9E87] opacity-20"
            style={{ clipPath: "polygon(50% 0%, 0% 100%, 100% 100%)" }}
          ></div>
          <h2 className="absolute top-20 left-20 font-['Cormorant_Garamond'] italic text-5xl text-[#C9A96E] opacity-60">
            From Nepal...
          </h2>
        </div>
        ...
        <div className="w-[100vw] h-full relative bg-gradient-to-b from-[#0A0816] to-[#1A0A2E]">
          <svg
            className="absolute top-20 right-32 w-24 h-24 text-[#F0E6D6] opacity-80"
            viewBox="0 0 24 24"
            fill="currentColor"
          >
        ...
        <div className="w-[100vw] h-full relative bg-gradient-to-b from-[#E8845A]/20 to-[#1A0A2E] flex items-end justify-center pb-20">
          <h2 className="absolute top-20 right-20 font-['Cormorant_Garamond'] italic text-5xl text-[#C9A96E] opacity-60">
            ...to Sydney
          </h2>
      ...
      <div className="absolute inset-0 pointer-events-none flex items-center justify-center z-50">
        <div
          ref={letterRef}
          className="w-full max-w-2xl bg-[#0A0816]/90 backdrop-blur-xl p-12 md:p-16 rounded-sm border border-[#C9A96E]/40 shadow-[0_0_50px_rgba(201,169,110,0.1)] relative opacity-0"
        >
          <p className="font-['Cormorant_Garamond'] italic text-xl md:text-2xl text-[#F0E6D6] leading-relaxed text-center">
            "Mero mutu ko tukri, its already been 10 months with you yet i still
            feel like I'm talking to you for the first time. What a wonderful
            soul you are and a Lucky MF i am to love you with my whole and open
            heart. You deserve the best best best things in life and never to
            worry bout anything yet you are there 9000 kms apart taking care of
            me. Its just a beginning, we have so much to live and learn but i
            just want to do those things with you and only you. idk i can even
            explain how much i care, love and want to spend time w you, but just
            know that i love you more than anything. Kaile khai ma karaunchu
            hola, i might be strict on you eventho you know, and understand the
            circumstance around you, but ion see you as19 years old saffy rather
            the lobed ma pachharreko kid ❤️😭 so hope yk its all love."
          </p>
